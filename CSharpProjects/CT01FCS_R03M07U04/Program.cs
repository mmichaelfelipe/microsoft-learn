...
// Unidade 04   - "Exercício – Garantir que os apelidos dos animais de estimação e suas descrições de personalidade estejam preenchidos" (25min)
// https://learn.microsoft.com/pt-br/training/modules/challenge-project-develop-branching-looping-structures-c-sharp/4-exercise-build-loop-verify-personality

// See https://aka.ms/new-console-template for more information
// Console.WriteLine("Hello, World!");

#region CT01FCS_R03M07U03
/*  Especificação
...
*/
#region Minha versão de código para o desafio
using System;
using System.IO;
...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "3":
            ...
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 0] is not "ID #: ")
                {
                    // validates the animal's age
                    if (ourAnimals[i, 2] is "Age: ?" or "Age: ")
                    {
                        validEntry = false;
                        do
                        {
                            ...
                            {
                                animalAge = readResult;
                                if (validEntry = int.TryParse(animalAge, out _))
                                    ourAnimals[i, 2] = $"Age: {animalAge}";
                            }
                        } while (validEntry == false);
                    }

                    // validates the animal's physical description
                    if (ourAnimals[i, 4] is "Physical description:  ?" or "Physical description: ")
                    {
                        validEntry = false;
                        do
                        {
                            ...
                            if (readResult is not null)
                            {
                                ...
                                if (validEntry = animalPhysicalDescription is not null && animalPhysicalDescription.Length > 0)
                                    ourAnimals[i, 4] = $"Physical description: {animalPhysicalDescription}";
                            }
                        } while (validEntry == false);
                    }
        ...
        case "4":
            ...
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 0] is not "ID #: ")
                {
                    // validates the animal's nickname
                    if (ourAnimals[i, 3] is "Nickname: ?" or "Nickname: ")
                    {
                        validEntry = false;
                        do
                        {
                            ...
                            if (readResult is not null)
                            {
                                ...
                                if (validEntry = animalNickname is not null && animalNickname.Length > 0)
                                    ourAnimals[i, 3] = $"Nickname: {animalNickname}";
                            }
                        } while (validEntry == false);
                    }

                    // validates the animal's personality
                    if (ourAnimals[i, 5] is "Personality: ?" or "Personality: ")
                    {
                        validEntry = false;
                        do
                        {
                            ...
                            if (readResult is not null)
                            {
                                ...
                                if (validEntry = animalPersonalityDescription is not null && animalPersonalityDescription.Length > 0)
                                    ourAnimals[i, 5] = $"Personality: {animalPersonalityDescription}";
                            }
                        } while (validEntry == false);
                    }
